<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <flux:modal.trigger name="ajout-education">
            <flux:button icon="plus" wire:click='resetForm'>Ajouter une étude</flux:button>
        </flux:modal.trigger>
    </div>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-body">
            ...
            <tbody>
                ...
                <tr
                    ...
                    <td class="px-6 py-2">
                        {{ Str::limit($education->description, 30) }}
                    </td>

                    <td class="px-6 py-2 flex justify-start gap-1">
                        <flux:modal.trigger name="editer-education">
                            <flux:button icon="pencil-square" size="sm" wire:click="edit({{ $education }})" />
                        </flux:modal.trigger>
                        <flux:button icon="trash" size="sm" wire:click="delete({{ $education->id }})" />
                    </td>
                </tr>
                @empty
                <tr>
    ...
    <flux:modal name="ajout-education" class="md:w-150" wire:ignore.self>
        <form wire:submit="save" class="space-y-6">
            <div>
                <flux:heading size="lg">Ajouter une étude</flux:heading>
            </div>
            <flux:input wire:model='school' label="Ecole ou Université" placeholder="Mon école ou université" />
            <flux:input wire:model='degree' label="Diplôme" placeholder="Mon diplôme" />
            <flux:input wire:model='field' label="Filière ou Option" placeholder="Ma filière ou option" />
            <flux:input wire:model='start_year' label="Date de début" type="number" placeholder="Date de début" />
            <flux:input wire:model='end_year' label="Date de fin" type="number" placeholder="Date de fin" />
            <flux:textarea wire:model='description' label="Description" placeholder="Ma description" />
            <div class="flex">
                <flux:spacer />
    ...
    <flux:modal name="editer-education" class="md:w-150" wire:ignore.self>
        <form wire:submit="update" class="space-y-6">
            <div>
                <flux:heading size="lg">Editer une étude</flux:heading>
            </div>
            <flux:input wire:model='school' label="Ecole ou Université" placeholder="Mon école ou université" />
            <flux:input wire:model='degree' label="Diplôme" placeholder="Mon diplôme" />
            <flux:input wire:model='field' label="Filière ou Option" placeholder="Ma filière ou option" />
